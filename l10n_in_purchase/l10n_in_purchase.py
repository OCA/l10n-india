...
class purchase_order(models.Model):
    ...
    def action_invoice_create(self):
        ...
        for order in self:
            if order.company_id.id != uid_company_id:
                ...
                self._context['force_company'] = order.company_id.id
                order = self
            pay_acc_id = order.partner_id.property_account_payable.id
            journal_ids = journal_obj.search([('type', '=', 'purchase'), ('company_id', '=', order.company_id.id)], limit=1)
            journal_ids = [rec.id for rec in journal_ids]
            if not journal_ids:
            ...
            for po_line in order.order_line:
                ...
                po_line.write({'invoiced': True, 'invoice_lines': [(4, inv_line_id.id)]})
            ...
            # compute the invoice
            inv_obj.button_compute([inv_id.id])
    ...
    @api.multi
    @api.depends('round_off','insurance', 'insurance_type', 'freight_type', 'freight', 'package_and_forwording_type', 'package_and_forwording', 'order_line','order_line.price_subtotal')
    def _amount_all(self):
        ...
        for order in self:
            ...
            for line in order.order_line:
                ...
                taxes = line.taxes_id.compute_all( price, line.product_qty, line.product_id, line.order_id.partner_id)
            ...
            untax_amount = cur.round(untax_amount)  - included_price            
            order_total = other_charges + tax_total + untax_amount + order.round_off
            
            self.amount_tax = tax_total
            self.amount_untaxed = untax_amount
            self.other_charges = other_charges
            self.amount_total = order_total
            
    
    #Need action : removed ,('include', 'Include in Price') from all options, need to remove rlated code to make it clean
    package_and_forwording_type = fields.Selection(selection=[('fix', 'Fix Amount'), ('percentage', 'Percentage'), ('per_unit', 'Per Unit'), ('actual', 'At actual')], string='Packaging & Forwarding Type', default='fix' ,states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    package_and_forwording = fields.Float(string='Packaging & Forwarding', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    insurance_type = fields.Selection(selection=[('fix', 'Fix Amount'), ('percentage', 'Percentage')], string='Insurance Type', default='fix',states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})  
    insurance = fields.Float(string='Insurance', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    delivery_id = fields.Many2one('mill.delivery', string='Mill Delivery', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    freight = fields.Float(string='Freight', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    freight_type = fields.Selection(selection=[('fix', 'Fix Amount'), ('percentage', 'Percentage'), ('per_unit', 'Per Unit'),('actual', 'At actual')], string='Freight Type',default='fix', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    round_off = fields.Float(string='Round Off', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]}, help="Round Off Amount")
    amount_untaxed = fields.Float(compute = _amount_all, digits_compute= dp.get_precision('Account'), string='Untaxed Amount',
    ...
    amount_package_and_forwording = fields.Float(string='Packaging & Forwarding', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    amount_insurance = fields.Float(string='Insurance', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    amount_freight = fields.Float(string='Freight', states={'confirmed':[('readonly', True)], 'approved':[('readonly', True)], 'done':[('readonly', True)]})
    
    @api.onchange('package_and_forwording_type','freight_type','insurance_type')
    def onchange_reset(self):
        if self.insurance_type in ('include',False):
            self.insurance = 0.0
        elif self.freight_type == 'actual':
            self.freight = 0.0
        elif self.package_and_forwording_type == 'actual':
            self.package_and_forwording = 0.0
...
#         
        for order in self:
            order_line_pool = self.env['purchase.order.line']
            res_vals = {
...
class purchase_order_line(models.Model):
    ...
    package_and_forwording = fields.Float(string='Packing Unit', default=0.0)
    insurance = fields.Float(string='Insurance Unit',default=0.0)
    freight = fields.Float(string='Freight Unit',default=0.0)
    
purchase_order_line()

class mill_delivery(models.Model):
    _name = 'mill.delivery'
    ...
    active = fields.Boolean(string='Active',default = True , help="If the active field is set to False, it will allow you to hide the mill delivery without removing it.")
